...
export default function MatchPending() {
  ...
  const load = async () => {
    ...
    setBusy(true);
    try {
      const data = await apiFetch<MatchItem[]>("/matches/outgoing");
      setItems(data);
    } catch (e: any) {
  ...
  return (
    <div className="container" style={{ maxWidth: 980 }}>
      <Panel>
        <div className="sectionTitle">Pending Matches</div>
        <p className="sectionDesc">People you’ve requested to match with.</p>

        {err && <div className="error" style={{ marginBottom: 12 }}>{err}</div>}
        {busy && <div className="smallMuted">Loading...</div>}

        {!busy && items.length === 0 && !err && (
          <div className="smallMuted">No pending matches.</div>
        )}
      </Panel>
